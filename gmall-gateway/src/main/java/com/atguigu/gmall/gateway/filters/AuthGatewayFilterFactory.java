...
//        return Arrays.asList("key","value","desc");
    ...
    public GatewayFilter apply(PathConfig config) {
        return new GatewayFilter() {
            ...
            public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
                ...
                String token = request.getHeaders().getFirst("token"); //异步请求情况下头信息获取token信息
                ...
                //3.判断token是否为空，为空重定向到登陆页面并拦截 ，以上都为空
                if (StringUtils.isBlank(token)){
                    //设置重定向相关信息
                ...
                try {
                    ...
                    //5.判断token信息，如果解析出现异常要重定向到登陆页面并拦截
