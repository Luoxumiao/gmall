...
public class WareSkuServiceImpl extends ServiceImpl<WareSkuMapper, WareSkuEntity> implements WareSkuService {
    ...
    public List<SkuLockVo> checkAndLock(List<SkuLockVo> lockVos, String orderToken) {
        ...
        // 判断是否所有商品锁定成功，如果有一个商品锁定失败，所有锁定成功的商品应该解锁库存  lockVo -> !lockVo.getLock() 锁定失败，需要解锁，获取解锁成功的商品
        ...
        // 所有商品锁定成功的情况下，在返回之前，应该把锁定信息缓存到redis中，以方便将来的某个时刻解锁对应的库存,哪个订单，需要通过浏览器地址传递订单编号
    ...
    private void checkLock(SkuLockVo lockVo){
        ...
        try {
            ...
            // 锁库存：更新库存表中的stock_lock字段。大数据提供库存接口，获取就近的库存id。这里取第一个库存。(就近原则，那个地址仓库离地址近发哪个)
