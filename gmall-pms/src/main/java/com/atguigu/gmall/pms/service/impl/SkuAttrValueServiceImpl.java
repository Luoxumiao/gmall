...
public class SkuAttrValueServiceImpl extends ServiceImpl<SkuAttrValueMapper, SkuAttrValueEntity> implements SkuAttrValueService {
    ...
    public List<SkuAttrValueEntity> querySearchSkuAttrValuesByCidAndSkuId(Long cid, Long skuId) {
        ...
        List<AttrEntity> attrEntities = attrMapper.selectList(new QueryWrapper<AttrEntity>().eq("search_type", 1).eq("category_id", cid));
    ...
    public List<SaleAttrValueVo> querySaleAttrValueBySpuId(Long spuId) {
        ...
        ArrayList<SaleAttrValueVo> attrValueVos = new ArrayList<>();
    ...
    public String querySaleAttrValuesMappingSkuIdBySpuId(Long spuId) {
        List<Map<String, Object>> maps = this.skuAttrValueMapper.querySaleAttrValuesMapppingBySkuId(spuId);
        if (CollectionUtils.isEmpty(maps)){
            return null;
        }

        //[{sku_id=1, attr_values=8G,128G,黑色}, {sku_id=2, attr_values=白色,8G,256G}]
        //把上面这个数据模型处理成下面这种结构
        //{'8G,128G,黑色': 1,'白色,8G,256G': 2}
